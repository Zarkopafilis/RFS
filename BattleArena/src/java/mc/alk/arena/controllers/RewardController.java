...
import com.rfslabs.tokenizer.TokenBridge;
	...
	public void giveRewards() {
		for (ArenaTeam t: event.getTeams()){
			for (ArenaPlayer player: t.getPlayers()){
				...
				if(event instanceof ArenaWinnersPrizeEvent){
					p.sendMessage("+ 40 Tokens!");
					TokenBridge.setTokens(p.getUniqueId(), TokenBridge.getTokens(p.getUniqueId()) + 40);
				}
